...
// addToDo('walk the dogs', 0, false, true);
// -------------------------------------------------------------------------------------------------
...
function displayToDo(event) {
    if (event.keyCode === 13 || event.target.classList.value === 'fa fa-plus-circle') {
        ...
        // check whether the input field is NOT empty
        if (toDo) {
...
//Targeting the dynamically create to do item
toDoList.addEventListener('click', evt => {
    if (evt.path[0].localName === 'p' || evt.path[0].localName === 'li' || evt.path[0].localName === 'ul')
        return;
